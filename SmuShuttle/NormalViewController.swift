...
class NormalViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        if self.location == "caStation" {
            ...
            let path = NSBundle.mainBundle().pathForResource("normalCa_sv", ofType: "json")
            let data = NSData(contentsOfFile: path!) //new code
            ...
            let path = NSBundle.mainBundle().pathForResource("normalTe_sv", ofType: "json")
            ...
            let nib = UINib(nibName: "OyStation", bundle:nil)
            timeTableView.registerNib(nib, forCellReuseIdentifier: "customCell")
            self.timeTableView.contentInset = UIEdgeInsetsMake(-2.0, 0.0, 0.0, 0.0)
            self.topBar.image = UIImage(named: "oy_top")
            let path = NSBundle.mainBundle().pathForResource("onyang_sv", ofType: "json")
            let data = NSData(contentsOfFile: path!) //new code
            jsons = JSON(data: data!, options: NSJSONReadingOptions.AllowFragments, error: nil)
            counts = jsons?.count
        }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if self.location == "caStation"{
            ...
            if (tmp!["천안역"].stringValue.rangeOfString("금X") != nil) {
                ...
                cell.label3.font = UIFont(name: cell.label3.font.fontName, size: 14)
            }
            ...
            if (tmp!["아산역Ktx 2"].stringValue.rangeOfString("금X") != nil) {
                ...
                cell.label5.font = UIFont(name: cell.label5.font.fontName, size: 14)
            }
            ...
            if (tmp!["아산캠퍼스도착"].stringValue.rangeOfString("금X") != nil) {
                ...
                cell.label6.font = UIFont(name: cell.label6.font.fontName, size: 14)
            }
            ...
            let cell: OyStationViewCell = self.timeTableView.dequeueReusableCellWithIdentifier("customCell") as! OyStationViewCell
            var tmp = jsons?.array?[indexPath.row]
            cell.label1.text = tmp!["아산캠퍼스출발"].stringValue
            cell.label2.text = tmp!["배방지역"].stringValue
            cell.label3.text = tmp!["온양터미널"].stringValue
