...
class SatViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        if self.location == "caStation" {
            ...
            let path = NSBundle.mainBundle().pathForResource("cheonSat", ofType: "json")
            ...
            let path = NSBundle.mainBundle().pathForResource("terminalSat", ofType: "json")
            let data = NSData(contentsOfFile: path!) //new code
            jsons = JSON(data: data!, options: NSJSONReadingOptions.AllowFragments, error: nil)
            counts = jsons?.count
